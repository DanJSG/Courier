...
def login(code_challenge, state, email, password):
    ...
    body = {'credentials': base64.b64encode(json.dumps(
        {'email': email, 'password': password}).encode()).decode()}
...
def save_account(user):
    ...
    refresh_token, refresh_cookie = request_refresh_token(
        auth_code, verifier, state)
    access_token, access_cookie = request_access_token(
